    	...
    	grid: {
	        ...
	        async onClick(event) {
	            ...
	            event.done(() => {
	                ...
	                let form = w2ui['adminForm'];
	                if (sel.length == 1) {
	                    form.recid = sel[0]
	                    form.record = w2ui['adminGrid'].get(form.recid);
	                    form.refresh()
	                    
	                    /*let len = w2ui['adminForm'].record.roles.length;
	                    let roles;
	                    for(let i=0; i<len; i++){
							if(roles == undefined){
...
let adminlayout = $('#layouts').w2layout(adminConfig.layouts);
let admingrid = $('#adminGrid').w2grid(adminConfig.grid);
let adminform = $('#adminForm').w2form(adminConfig.form);
    ...
    w2popup.open({
		...
		onOpen: function(event){
			event.onComplete = function () {
		        ...
		        adminlayout.html('left', admingrid)
		        adminlayout.html('main', adminform)
            }
		},
